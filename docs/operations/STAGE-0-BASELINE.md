...
- Branch Neon `homologacao` criada como filha da `main`, sem exclusão automática e ainda desconectada da Vercel.
- Não existe agendamento automático de snapshots.
- Não foi encontrado ambiente formal de homologação.
...
- conectar a branch Neon isolada a uma homologação protegida, sem reutilizar as variáveis produtivas;
