...
assert.ok(index.indexOf("crm-ranking.js?v=10.3-person-unification-r1") < index.indexOf("app.js?v=10.3.1-sync-confirmation-r1"));
assert.match(index, /Versão 10\.3\.1/);
assert.match(app, /async function saveUser[\s\S]*?await persistAndConfirm\("config"\)/);
assert.match(app, /async function saveProtocol[\s\S]*?await persistAndConfirm\(releasesProject/);
assert.match(app, /async function saveInstallation[\s\S]*?await persistAndConfirm\("projetos"\)/);
assert.match(app, /Usuário gravado na nuvem/);
assert.match(app, /Protocolo gravado na nuvem/);
assert.match(app, /Serviço gravado na nuvem/);
