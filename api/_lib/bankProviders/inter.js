...
// Logica pura (normalizacao, chamadas HTTP) do provider do Banco Inter,
// compartilhada entre backend-integration/ (execucao local, certificado em
// arquivo) e api/bank/inter.js (Vercel, certificado direto em env var). So a
// construcao do https.Agent muda entre os dois ambientes - por isso ela entra
// como parametro em vez de fixa no modulo.
...
function createInterProvider({ buildAgent, isConfigured }) {
  let tokenCache = null;

  function assertConfigured() {
    if (!isConfigured()) {
      throw new Error("Configure INTER_CLIENT_ID, INTER_CLIENT_SECRET e certificado/chave do Inter.");
    }
  }
  ...
  return { fetchStatement, fetchBalance, fetchInvestmentBalance, isConfigured };
}
...
// Ambiente local (backend-integration/): certificado/chave em arquivos.
...
function buildAgentFromPaths() {
  const options = { rejectUnauthorized: process.env.INTER_REJECT_UNAUTHORIZED !== "false" };

  if (process.env.INTER_PFX_PATH) {
    options.pfx = fs.readFileSync(process.env.INTER_PFX_PATH);
    if (process.env.INTER_CERT_PASSPHRASE) options.passphrase = process.env.INTER_CERT_PASSPHRASE;
  } else {
  ...
  const trustedCertificates = buildAgentOptions();
  if (process.env.INTER_CA_PATH) trustedCertificates.push(fs.readFileSync(process.env.INTER_CA_PATH, "utf8"));
  options.ca = trustedCertificates;

  return new https.Agent(options);
}

// Ambiente Vercel (api/bank/inter.js): certificado/chave direto no conteudo da
// env var (nao ha filesystem persistente para apontar caminho).
function isConfiguredFromEnvContent() {
  return Boolean(
    process.env.INTER_CLIENT_ID &&
      process.env.INTER_CLIENT_SECRET &&
      (process.env.INTER_CERT || process.env.INTER_PFX_BASE64) &&
      (process.env.INTER_KEY || process.env.INTER_PFX_BASE64)
  );
}
...
function buildAgentFromEnvContent() {
  ...
  if (process.env.INTER_PFX_BASE64) {
    options.pfx = Buffer.from(process.env.INTER_PFX_BASE64, "base64");
    if (process.env.INTER_CERT_PASSPHRASE) options.passphrase = process.env.INTER_CERT_PASSPHRASE;
  } else {
    options.cert = process.env.INTER_CERT;
    options.key = process.env.INTER_KEY;
    if (process.env.INTER_CERT_PASSPHRASE) options.passphrase = process.env.INTER_CERT_PASSPHRASE;
  }

  const trustedCertificates = buildAgentOptions();
  if (process.env.INTER_CA) trustedCertificates.push(process.env.INTER_CA);
  options.ca = trustedCertificates;
...
function getErrorDetail(parsed) {
  ...
  if (parsed.raw) return String(parsed.raw).replace(/\s+/g, " ").trim().slice(0, 300);
  return "";
}
...
function normalizeInterBalance(data) {
  ...
  const rawBalance = firstValue(source, [
    "disponivel", "saldoDisponivel", "saldoAtual", "saldo", "valor", "availableBalance", "balance",
  ]);
...
function normalizeInterInvestments(data) {
  ...
  const directValue = firstValue(source, [
    "saldoInvestimentos", "valorAtual", "valorBruto", "valorLiquido", "saldoBruto", "saldoLiquido", "total", "amount",
  ]);
...
function sumInvestmentItems(source) {
  const arrays = [
    source.investimentos, source.aplicacoes, source.posicoes, source.items,
